...
<div class="max-w-3xl mx-auto space-y-6">
    ...
    <div class="flex items-center justify-between">
        <div class="flex items-center space-x-3">
            ...
            <div>
                <h1 class="text-xl font-bold text-slate-900">Edit Data Santri</h1>
                <p class="text-xs text-slate-500 mt-0.5">Perbarui informasi data induk santri {{ $santri->nama_lengkap }}.</p>
            </div>
        </div>
    </div>

    <!-- Main Form Card -->
    <form action="{{ route('santri.update', $santri->id) }}" method="POST" class="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
        @csrf
        @method('PUT')
            ...
            <div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <input type="text" name="nis" id="nis" value="{{ old('nis', $santri->nis) }}" required placeholder="Contoh: 2024001"
                    ...
                    <div>
                        ...
                        <input type="text" name="nama_lengkap" id="nama_lengkap" value="{{ old('nama_lengkap', $santri->nama_lengkap) }}" required placeholder="Masukkan nama lengkap santri"
                    ...
                    <div>
                        ...
                        <input type="number" name="tahun_masuk" id="tahun_masuk" value="{{ old('tahun_masuk', $santri->tahun_masuk) }}" required min="2000" max="2100"
                ...
                <div class="mt-4">
                    ...
                    <textarea name="alamat" id="alamat" rows="3" placeholder="Alamat lengkap orang tua/wali..."
                              class="w-full px-3 py-2 text-xs border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 text-slate-800 placeholder-slate-400">{{ old('alamat', $santri->alamat) }}</textarea>
                </div>
            </div>
                        ...
                        @else
                            <select name="tingkat_id" id="tingkat_id" required
                                ...
                                @foreach($tingkatList as $tingkat)
                                    <option value="{{ $tingkat->id }}" {{ old('tingkat_id', $santri->tingkat_id) == $tingkat->id ? 'selected' : '' }}>
                                        {{ $tingkat->nama }}
                                    </option>
                                @endforeach
                            </select>
